...
import { getPatientByMRN, getUsers, createPatientUser } from '../../lib/dummyDatabase'
import { getUserById, setUserPassword } from '../../lib/dummyAuth'

type Step = 'identify' | 'password' | 'set-password'

export const PatientAccess: React.FC = () => {
  const { signIn } = useAuth()
  const [step, setStep] = useState<Step>('identify')
  const [identifier, setIdentifier] = useState('') // MRN or email
  ...
  const handleIdentify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!input) throw new Error('Please enter patient ID or email')
      if (input.includes('@')) {
        const { data } = await getUsers()
        const found = (data || []).find((u: any) => u.role === 'patient' && u.email.toLowerCase() === input.toLowerCase())
        if (!found) throw new Error('No patient account with this email')
        uid = found.id
        uEmail = found.email
      } else {
        const { data } = await getPatientByMRN(input)
        if (!data) throw new Error('Patient ID not found')
        if (data.user_id) {
          const u = getUserById(data.user_id)
          if (u && u.role === 'patient') {
            uid = u.id
            uEmail = u.email
          }
        ...
        if (!uid) {
          throw new Error('This patient has no account yet. Ask your care team to add an email for login.')
        }
      }
      setUserId(uid)
      setEmail(uEmail)
      ...
      setError(err.message || 'Unable to identify patient')
    } finally {
      setLoading(false)
    }
  }
      ...
      {step === 'identify' && (
        <form onSubmit={handleIdentify} className="space-y-6">
          <div className="text-center">
            <UserSearch className="w-8 h-8 inline text-blue-600" />
            <h2 className="text-xl font-semibold mt-2">Patient Access</h2>
            <p className="text-gray-600 text-sm">Enter your Patient ID (MRN) or Email</p>
          </div>
          <input
            type="text"
            value={identifier}
            ...
            placeholder="MRN-001234 or you@example.com"
          />
